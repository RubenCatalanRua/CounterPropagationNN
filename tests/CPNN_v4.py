...
def validate_hyperparameters(**kwargs):
    ...
    # Validate Grossberg hidden layers hyperparameters
    grossberg_hidden_layers = kwargs.get("grossberg_hidden_layers", 0)
    if not isinstance(grossberg_hidden_layers, int) or grossberg_hidden_layers < 0:
        raise ValueError("grossberg_hidden_layers must be a non-negative integer.")
    grossberg_hidden_size = kwargs.get("grossberg_hidden_size", None)
    if grossberg_hidden_layers > 0:
        if grossberg_hidden_size is None or not (isinstance(grossberg_hidden_size, int) and grossberg_hidden_size > 0):
            raise ValueError("grossberg_hidden_size must be a positive integer when grossberg_hidden_layers > 0.")
...
class CounterPropagationNetwork(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, init_method="xavier_uniform",
                 grossberg_hidden_layers=0, grossberg_hidden_size=None):
        super(CounterPropagationNetwork, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.output_size = output_size
        self.grossberg_hidden_layers = grossberg_hidden_layers

        # Initialize Kohonen (winner-take-all) weights.
        self.kohonen_weights = nn.Parameter(torch.empty(hidden_size, input_size))

        # Initialize Grossberg mapping:
        # If no extra hidden layers, use a single linear mapping (custom update)
        if grossberg_hidden_layers == 0:
            self.grossberg_weights = nn.Parameter(torch.empty(output_size, hidden_size))
        else:
            if grossberg_hidden_size is None:
                grossberg_hidden_size = hidden_size
            layers = []
            # First layer: from input (winner one-hot) to hidden size
            layers.append(nn.Linear(hidden_size, grossberg_hidden_size))
            layers.append(nn.ReLU())
            for _ in range(grossberg_hidden_layers - 1):
                layers.append(nn.Linear(grossberg_hidden_size, grossberg_hidden_size))
                layers.append(nn.ReLU())
            # Final layer to output
            layers.append(nn.Linear(grossberg_hidden_size, output_size))
            self.grossberg_net = nn.Sequential(*layers)
    ...
    def forward(self, x):
        if x.dim() > 2:
            x = x.view(x.size(0), -1)
        # Compute distances between input and Kohonen weights
        ...
        # Compute output through Grossberg mapping
        if self.grossberg_hidden_layers == 0:
            output = torch.matmul(winner_one_hot, self.grossberg_weights.t())
        else:
            output = self.grossberg_net(winner_one_hot)
    ...
    def train_grossberg(self, x, y, optimizer, learning_rate=0.1):
        # Get the winner indices from forward pass
        ...
        if self.grossberg_hidden_layers == 0:
            ...
            # Standard gradient descent on the MLP mapping
            optimizer.zero_grad()
            hidden_indices = torch.arange(self.hidden_size, device=x.device).float()
            winner_one_hot = torch.zeros(batch_size, self.hidden_size, device=x.device)
            winner_one_hot.scatter_(1, winner_indices.unsqueeze(1), 1)
    ...
    def fit(self, train_loader, val_data=None, config=None):
        ...
        if config.get("grossberg_optimizer_type", "sgd").lower() == "adam":
            if self.grossberg_hidden_layers == 0:
                ...
                grossberg_optimizer = torch.optim.Adam(self.grossberg_net.parameters(), lr=grossberg_lr)
        else:
            if self.grossberg_hidden_layers == 0:
...
class CPNNBase(ABC):
    ...
    def __init__(
        ...
        ae_lr_scheduler,
        grossberg_hidden_layers,
        grossberg_hidden_size
    ):
        self.input_size = input_size
        self.hidden_size = hidden_size
        ...
        self.grossberg_hidden_layers = grossberg_hidden_layers
        self.grossberg_hidden_size = grossberg_hidden_size
...
class CPNNClassifier(CPNNBase):
    def __init__(
        ...
        grossberg_hidden_layers=0,
        grossberg_hidden_size=None
    ):
        super().__init__(
    ...
    def fit(self, X, y):
        ...
        validate_hyperparameters(
            ...
            grossberg_hidden_layers=self.grossberg_hidden_layers,
            grossberg_hidden_size=self.grossberg_hidden_size
        )
