...
def validate_hyperparameters(**kwargs):
    ...
    Expected keys:
     - kohonen_lr, grossberg_lr: positive floats.
     - max_epochs, batch_size, patience, log_interval: positive integers.
     - use_autoencoder: boolean.
     - If use_autoencoder is True, then ae_dim, ae_epochs, ae_lr, ae_hidden_layers must be valid.
     - If "input_size" is provided, then ae_dim must be smaller than input_size.
     - distance_metric must be one of ['euclidean', 'manhattan', 'cosine'].
     - neighborhood_function must be one of ['gaussian', 'rectangular', 'triangular'].
    ...
    if kwargs.get("use_autoencoder", False):
        ...
        if "input_size" in kwargs and kwargs["ae_dim"] >= kwargs["input_size"]:
            print('ae_dim:', kwargs["ae_dim"])
            print('input_size:', kwargs["input_size"])
...
class FlexibleAutoencoder(nn.Module):
    """
    A flexible autoencoder network with configurable hidden layers, activation functions,
    and an optional convolutional variant.

    If conv is True, the input is expected to be a flattened vector and will be reshaped
    to a square image (or as close as possible) before being processed.
    """
    def __init__(self, input_size, encoding_dim, hidden_layers=1, hidden_size=128, activation='relu', conv=False):
        ...
        if conv:
            ...
            # We try to make it as square as possible.
            side = int(np.sqrt(input_size))
            if side * side != input_size:
                # Handle cases where input_size is not a perfect square
                # You might want to add padding or resize logic depending on the application
            ...
            if hidden_layers == 0:
                ...
                decoder_layers.append(nn.Sigmoid()) # Output should be in the range [0, 1] for MSE loss
    ...
    def forward(self, x):
        if self.conv:
            # Reshape the flat input (batch, input_size) into (batch, 1, side, side)
            x = x.view(-1, 1, self.side, self.side)
        encoded = self.encoder(x)
        decoded = self.decoder(encoded)
        if self.conv:
            # Flatten the decoded output back to (batch, input_size)
...
def train_autoencoder(autoencoder, train_data, val_data=None, epochs=50, lr=0.001, batch_size=128,
    ...
    """
    Train the autoencoder using MSE loss with optional validation and early stopping.

    Args:
        autoencoder (nn.Module): Autoencoder model.
        train_data (Tensor): Training data.
        val_data (Tensor, optional): Validation data.
    ...
    Returns:
        autoencoder (nn.Module): Trained autoencoder.
    """
    if lr <= 0:
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=patience // 2, factor=0.5, verbose=verbose) # Improved early stopping
...
class CounterPropagationNetwork(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size, init_method="xavier_uniform"):
        ...
        # Set default distance metric and neighborhood function.
    ...
    def forward(self, x):
        ...
        if self.distance_metric == 'euclidean':
            ...
            # Normalize input and weights for cosine similarity
            x_norm = F.normalize(x, p=2, dim=1)
            weights_norm = F.normalize(self.kohonen_weights, p=2, dim=1)
            distances = 1 - torch.matmul(x_norm, weights_norm.t())
        else:
            distances = torch.cdist(x, self.kohonen_weights)  # Default to Euclidean
    ...
    def train_kohonen(self, x, learning_rate=0.1, neighborhood_size=3):
        """
        Update Kohonen weights using a neighborhood function.
        """
        distances = torch.cdist(x, self.kohonen_weights)
        winner_indices = torch.argmin(distances, dim=1)
        ...
        with torch.no_grad():
            for i in range(x.size(0)):
                ...
                input_vector = x[i]
                # Calculate neighborhood influence for all Kohonen neurons
                ...
                # Update weights of the neighboring neurons
                update = learning_rate * influence.unsqueeze(1) * (input_vector - self.kohonen_weights)
                self.kohonen_weights.add_(update)
            self.kohonen_weights.copy_(F.normalize(self.kohonen_weights, p=2, dim=1)) # Normalize after each batch
    ...
    def fit(self, train_loader, val_data=None, config=None):
        ...
        if config.get("kohonen_lr_scheduler") is not None:
            kohonen_lr_scheduler = config["kohonen_lr_scheduler"](
                optimizer=torch.optim.SGD([self.kohonen_weights], lr=config["kohonen_lr"]),
                last_epoch=-1
            )
        if config.get("grossberg_lr_scheduler") is not None:
            grossberg_lr_scheduler = config["grossberg_lr_scheduler"](
                optimizer=torch.optim.SGD([self.grossberg_weights], lr=config["grossberg_lr"]),
                last_epoch=-1
            )
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if kohonen_lr_scheduler is not None:
                kohonen_lr_scheduler.step()
            if grossberg_lr_scheduler is not None:
                grossberg_lr_scheduler.step()
            ...
            if verbose and log_interval and epoch % log_interval == 0 and val_data is not None:
                ...
                logging.info(f"Epoch {epoch}/{max_epochs} | Validation Accuracy: {acc:.2f}%")
                if early_stopping:
                    if acc > best_val_acc:
                        best_val_acc = acc
                        epochs_no_improve = 0
                        ...
                        if acc < 0.5 * best_val_acc:
                            logging.warning("Early stopping: validation accuracy dropped to less than half of the best value.")
                            break
                        if epochs_no_improve >= patience:
                            logging.warning(f"Early stopping: no improvement for {patience} epochs.")
                            break
                self.train()
...
class CPNNClassifier:
    """
    Wrapper for the CounterPropagation Network (CPNN) that integrates optional autoencoder preprocessing,
    device selection, hyperparameter validation, training, prediction, saving, and loading.
    """
    def __init__(self, input_size, hidden_size=100, init_method="xavier_uniform",
        ...
        # Validate hyperparameters. 'input_size' must be provided later via fit; here we supply a dummy value for validation.
        validate_hyperparameters(kohonen_lr=kohonen_lr, grossberg_lr=grossberg_lr,
                                   ...
                                   log_interval=log_interval, input_size=input_size,  # Dummy value; will be updated in fit.
    ...
    def fit(self, X, y):
        """
        Fit the CPNN model on data X with labels y.
        ...
        # Convert data to tensors and move to the selected device.
        ...
        # Update hyperparameter validation with the correct input size.
        ...
        # Optionally preprocess with an autoencoder.
        ...
        # Create DataLoader.
        train_dataset = torch.utils.data.TensorDataset(X_train, y_tensor)
        loader = torch.utils.data.DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True)

        # Instantiate the CPNN.
    ...
    def predict_proba(self, X):
        """
        Predict class probabilities for the input X.
        """
        self.cpnn_.eval()
        X_tensor = torch.tensor(X, dtype=torch.float32).to(self.device)
        if self.use_autoencoder:
    ...
    def load(self, filepath):
        ...
        # Manually set the LR schedulers to None after loading, as their state is not typically saved.
        # The user should re-initialize them if needed.
